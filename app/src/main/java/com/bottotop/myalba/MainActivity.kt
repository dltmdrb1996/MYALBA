...
//    private val viewModel: SharedViewModel by viewModels()
    ...
    override fun onOptionsItemSelected(item: MenuItem) = when (item.itemId) {
        ...
        R.id.menu_notification -> {
            if (NavigationRouter.currentState != NavigationTable.Notification) {
                navigator.navController.deepLinkNavigateTo(
                    DeepLinkDestination.Notification(
                        SocialInfo.id
                    )
                )
            }
    ...
    private fun setBottomNavAction(navController: NavController) {
        binding.navViewFragment.setOnItemSelectedListener {
            when (it.itemId) {
                R.id.member_flow -> if (NavigationRouter.currentState != NavigationTable.Member) navigateToFlow(
                    NavigationFlow.MemberFlow("member")
                )
                R.id.community_flow -> if (NavigationRouter.currentState != NavigationTable.Community) navigateToFlow(
                    NavigationFlow.CommunityFlow("community")
                )
                R.id.home_flow -> if (NavigationRouter.currentState != NavigationTable.Home) navigateToFlow(
                    NavigationFlow.HomeFlow("home")
                )
                R.id.schedule_flow -> if (NavigationRouter.currentState != NavigationTable.Schedule) navigateToFlow(
                    NavigationFlow.ScheduleFlow("schedule")
                )
                R.id.asset_flow -> if (NavigationRouter.currentState != NavigationTable.Asset) navigateToFlow(
                    NavigationFlow.AssetFlow("asset")
                )
            }
    ...
    private fun observeNavigation(navController: NavController) {
        navController.addOnDestinationChangedListener { _, destination, arguments ->
            ...
            NavigationRouter.arg = (arguments?.get("msg") ?: "none") as String
            initBar(NavigationRouter.currentState)
        }
    }

    private fun initBar(navigationTable: NavigationTable){
        when (navigationTable) {
    ...
    override fun onBackPressed() {
        ...
        when (NavigationRouter.currentState) {
            is NavigationTable.Asset, NavigationTable.Community, NavigationTable.Member, NavigationTable.Schedule -> {
                navigateToFlow(NavigationFlow.HomeFlow("none"))
            }
